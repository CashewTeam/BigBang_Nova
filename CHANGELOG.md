...
#### 新增功能
- OCR 范围选择页新增二维码识别：普通文本可直接进入大爆炸分词，网页链接可选择浏览器打开，应用调用链接会拉起对应应用，WiFi 二维码可查看 SSID 和复制密码。
- 微信支付 `wxp://` 二维码会提示使用微信扫一扫，并可直接打开微信。
- 无法由系统应用打开的链接会显示原文，可复制或交给大爆炸处理。
- 扫码入口现同时支持二维码和常见一维条形码。
- WiFi 二维码识别结果支持复制 SSID、密码或完整网络信息。
